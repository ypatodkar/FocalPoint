...
| You | Teammate |
|-----|----------|
| React chat UI | FastAPI backend setup |
| WebGazer.js integration | Gemini API integration |
| Zone mapping on response DOM | Reward function computation |
| Gaze event detection (re-read, skim, skip) | MongoDB schema + read/write |
| Working memory state management | System prompt builder |
| Frontend polish + demo flow | Meta-agent (Antigravity) |
